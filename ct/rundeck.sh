...
export PCT_OPTIONS="
  ...
  $PW
"
bash -c "$(wget -qLO - https://raw.githubusercontent.com/tommasopoerio/Proxmox/main/ct/create_lxc.sh)" || exit

msg_info "Starting LXC Container"
pct start $CTID
msg_ok "Started LXC Container"

lxc-attach -n $CTID -- bash -c "$(wget -qLO - https://raw.githubusercontent.com/tommasopoerio/Proxmox/main/setup/rundeck-install.sh)" || exit

IP=$(pct exec $CTID ip a s dev eth0 | sed -n '/inet / s/\// /p' | awk '{print $2}')

msg_ok "Completed Successfully!\n"
echo -e "Rundeck should be reachable by going to the following URL.
         ${BL}http://${IP}:4440 ${CL} \n"
